...
def preprocess(datadir, outdir):
    ...
    dists.build(FLAGS.input_dir + 'dac_sample.txt', continous_features)
    #pool.apply(dists.build, args=(FLAGS.input_dir + 'dac_sample.txt', continous_features,))

    dicts = CategoryDictGenerator(len(categorial_features))
    dicts.build(FLAGS.input_dir + 'dac_sample.txt', categorial_features, cutoff=FLAGS.cutoff)
    #pool.apply(dicts.build, args=(FLAGS.input_dir + 'dac_sample.txt', categorial_features,))
    ...
    with open(FLAGS.output_dir + 'tr.libsvm', 'w') as out_train:
        with open(FLAGS.output_dir + 'va.libsvm', 'w') as out_valid:
            with open(FLAGS.input_dir + 'dac_sample.txt', 'r') as f:
                for line in f:
                    features = line.rstrip('\n').split('\t')
